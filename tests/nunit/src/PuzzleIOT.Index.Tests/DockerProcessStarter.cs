	...
	public class DockerProcessStarter
	{
		...
		protected string RunDockerProcess(string command)
		{
			...
			if (!IsMockDocker)
			{
				...
				fullCommand += " -v " + WorkingDirectory + ":~/workspace/PuzzleIOT/Index -v /var/run/docker.sock:/var/run/docker.sock compulsivecoder/ubuntu-arm-iot-mono";
			}
			fullCommand += " " + command;
		...
		public string RunDockerBash(string internalCommand)
		{
			...
			var cdCommand = "cd ~/workspace/PuzzleIOT/Index &&";
