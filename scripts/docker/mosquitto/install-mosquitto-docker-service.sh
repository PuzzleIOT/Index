...
sudo cp -f puzzleiot-mosquitto-docker.service /lib/systemd/system/puzzleiot-mosquitto-docker.service && \
echo "Setting service permissions" && \
sudo chmod 644 /lib/systemd/system/puzzleiot-mosquitto-docker.service && \

echo "Enabling service" && \
sudo sh $SYSTEMCTL_SCRIPT enable puzzleiot-mosquitto-docker.service && \
echo "Starting/restarting service" && \
sudo sh $SYSTEMCTL_SCRIPT restart puzzleiot-mosquitto-docker.service && \
