...
# Create device info
sh create-device-info.sh switch/SoilMoistureSensorCalibratedPump $DEVICE_LABEL $DEVICE_NAME $DEVICE_PORT && \

# Set up MQTT bridge service
sh create-mqtt-bridge-service.sh switch $DEVICE_NAME $DEVICE_PORT && \

# Set up update service
sh create-updater-service.sh switch uno $DEVICE_NAME $DEVICE_PORT && \

# Uploading sketch
sh upload-switch-uno-sketch.sh $DEVICE_PORT && \
